...
class CPDatasetTest(data.Dataset):
    ...
    def get_agnostic(self, im, im_parse, pose_data):
        ...
        length_b = np.linalg.norm(pose_data[13] - pose_data[10])
        point = (pose_data[10] + pose_data[13]) / 2
        pose_data[10] = point + (pose_data[10] - point) / length_b * length_a
        pose_data[13] = point + (pose_data[13] - point) / length_b * length_a

        if(pose_data[10][1]>pose_data[13][1]):
            pose_data[13][1] = pose_data[10][1]
        if(pose_data[10][1]<pose_data[13][1]):
            pose_data[10][1] = pose_data[13][1]
        ...
        agnostic_draw.line([tuple(pose_data[i]) for i in [2, 10]], 'gray', width=r*6)
        agnostic_draw.line([tuple(pose_data[i]) for i in [5, 13]], 'gray', width=r*6)
        # agnostic_draw.line([tuple(pose_data[i]) for i in [10, 13]], 'gray', width=r*6)
        agnostic_draw.polygon([tuple(pose_data[i]) for i in [2, 5, 13, 10]], 'gray', 'gray')
